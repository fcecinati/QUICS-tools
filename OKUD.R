# Ordinary Kriging with Uncertain Data (OKUD)
# ordinary kriging interpolation with spatially variable errors:
# 
...
OKUD <- function(data, newdata, variogr, model, errors){
  # data = SpatialPointsDataFrame with the measurements
  # newdata = SpatialPointsDataFrame with the measurements
  ...
  for (x in 1:newdata@grid@cells.dim[1]){
    for (y in 1:newdata@grid@cells.dim[2]) {
      ...
      #Multiply inverce C and D to get the weights
      W <- Cinv%*%D
      
      # multiply the weights to the values
      w <- t(W[1:(length(W)-1)])
      pred[x,y] <- w%*%as.matrix(data@data)
